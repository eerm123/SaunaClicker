...
int main(int argc, char *argv[]) {
    ...
    if (!background) {
        cerr << "Ei suutnud luua backgroundi: " << IMG_GetError() << endl;
    ...
    SDL_Texture *backgroundTexture = SDL_CreateTextureFromSurface(renderdaja, background);
    SDL_FreeSurface(background);
    if (!backgroundTexture) {
        cerr << "Ei suutnud luua background tekstuuri: " << SDL_GetError() << endl;
        SDL_DestroyRenderer(renderdaja);
        SDL_DestroyWindow(aken);
        SDL_Quit();
        return 1;
    }
    cout << "Background loodud" << endl;

    SDL_Surface *siga = IMG_Load("siga.png");
    if (!siga) {
        cerr << "Ei suutnud luua background tekstuuri: " << SDL_GetError() << endl;
        SDL_DestroyRenderer(renderdaja);
        SDL_DestroyWindow(aken);
    ...
    SDL_Texture *sigaTexture = SDL_CreateTextureFromSurface(renderdaja, siga);
    SDL_FreeSurface(siga);
    if (!sigaTexture) {
        cerr << "Ei suutnud luua background tekstuuri: " << SDL_GetError() << endl;
        SDL_DestroyRenderer(renderdaja);
        SDL_DestroyWindow(aken);
    ...
    cout << "siga loodud" << endl;

    SDL_Surface *kraadiklaas = IMG_Load("vaikekraadiklaas.png");
    if (!kraadiklaas) {
        cerr << "Ei suutnud luua background tekstuuri: " << SDL_GetError() << endl;
        SDL_DestroyRenderer(renderdaja);
        SDL_DestroyWindow(aken);
    ...
    SDL_Texture *kraadiklaasTexture = SDL_CreateTextureFromSurface(renderdaja, kraadiklaas);
    SDL_FreeSurface(kraadiklaas);
    if (!kraadiklaasTexture) {
        cerr << "Ei suutnud luua background tekstuuri: " << SDL_GetError() << endl;
        SDL_DestroyRenderer(renderdaja);
        SDL_DestroyWindow(aken);
        SDL_Quit();
    ...
    cout << "kraadiklaas loodud" << endl;

    automaatClicker.rect = {890, 50, 375, 50}; // Vasakule üles kast (automaatklikeri jaoks)
    automaatClicker2.rect = {890, 100, 375, 50};
    automaatClicker3.rect = {890, 150, 375, 50};
    automaatClicker4.rect = {890, 200, 375, 50};
    ...
    int kogus3 = 0;
    int kogus4 = 0;
    keriseRect.rect = {AKNA_LAIUS / 2 + 20, 500, 180, 200};
    olukast.rect = {400, 400, 100, 100};
    olukast.enabled = false;


    SDL_Rect poodRect = {850, 10, 80, 40}; // Vasakule üles kast (Poe jaoks kast)
    SDL_Rect sigaRect = {sigaX, sigaY, sigaWidth, sigaHeight};
    SDL_Rect kraadRect = {kraadX, kraadY, kraadWidth, kraadHeight};
    ...
    while (!välju) {
        ...
        while (SDL_PollEvent(&sündmus)) {
            if (sündmus.type == SDL_QUIT) {
                ...
                if (sündmus.button.button == SDL_BUTTON_LEFT) {
                    ...
                    } else if (kursorClickeril(olukast, hiirX, hiirY) && olukast.enabled) {
        ...
        SDL_RenderCopy(renderdaja, backgroundTexture, NULL, NULL);
        SDL_RenderCopy(renderdaja, sigaTexture, NULL, &sigaRect);
        SDL_RenderCopy(renderdaja, kraadiklaasTexture, NULL, &kraadRect);
        ...
        renderText(font, skooriTekst.str(), {255, 255, 255, 255}, AKNA_LAIUS / 2 - 190, 610);

        // Näita saunaraha
        stringstream saunarahaTekst;
        saunarahaTekst << "Saunaraha $: " << saunaraha;
        renderText(font, saunarahaTekst.str(), {22, 156, 53, 255}, saunarahaX, saunarahaY);
    ...
    SDL_DestroyTexture(sigaTexture);
    SDL_DestroyTexture(kraadiklaasTexture);
    SDL_DestroyRenderer(renderdaja);
    SDL_DestroyWindow(aken);
    SDL_Quit();
    return 0;
